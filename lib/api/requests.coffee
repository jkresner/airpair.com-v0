...
###############################################################################
  ...
  update: (req, res, next) =>
    ...
    @model.findOne search, (e, r) =>
      ...
      if data.status is "holding" && (!data.owner?||data.owner=='')
        evts.push @svc.newEvent req, "send received email"
      ...
      if data.status is "canceled" && !data.canceledDetail
        ...
        evts.push @svc.newEvent req, "canceled"
      ...
      if data.status is "incomplete" && !data.incompleteDetail
        ...
        evts.push @svc.newEvent req, "incomplete"

      for s in req.body.suggested

        if !s.events?
          data.status = "review"
          reqEvt = @svc.newEvent req, "suggested #{s.expert.username}"
          evts.push reqEvt
          ...
          s.events = [@svc.newEvent req, "first contacted"]
      ...
      if r.suggested?
        for s in r.suggested
          ...
          if !match?
            evts.push @svc.newEvent req, "removed suggested #{s.expert.username}"

      if evts.length is 0
        evts.push @svc.newEvent req, "updated"
  ...
  updateSuggestionByCustomer: (req, res, next, r) =>
    ...
    sug.events.push @svc.newEvent(req, "customer updated")
    ...
    data.events.push @svc.newEvent req, "customer expert review", ups
